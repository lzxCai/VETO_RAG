...
def merge_broken_sentence_lines(lines: List[str]) -> List[str]:
    """
...
def post_process_full_text(text: str) -> str:
    """
    对合并后的全文做后处理：
    1. 在章节标题前补换行
    2. 在第X条前补换行
...
def merge_pages(pages: List[Dict]) -> str:
    ...
    # 合并断裂句子
    lines = merge_broken_sentence_lines(lines)
    ...
    # 最后补关键换行
    final_text = post_process_full_text(final_text)
